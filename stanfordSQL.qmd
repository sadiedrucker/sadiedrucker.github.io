...
```{r}
#| fig-alt: "Line plot of average driver age by hour of traffic stops in North Carolina. Ages decrease at night, with late-night drivers around 30–32 years old, and daytime drivers around 34–37 years old."
library(tidyverse)
...
age_of_stops |>
  ...
  labs(
    ...
    title = "Average Age of Drivers by Hour of Traffic Stops in North Carolina"
  ) +
  theme_minimal()
```
...
```{r}
#| fig-alt: "Horizontal bar plot of arrest rates by race in Raleigh traffic stops. Hispanic drivers have the highest rate at 4.92%, White drivers the lowest at 2.13%."
library(forcats)
...
ggplot( aes(x = fct_reorder(race, pct_arrest), y = pct_arrest)) +
  ...
  labs(
    title = "Arrest Rate by Race in Raleigh, NC Traffic Stops",
    x = "Race",
...
SELECT 
    ...
    (SELECT COUNT(*) 
     FROM nc_fayetteville_2020_04_01 
...
```{r}
#| fig-alt: "Vertical bar plot showing the share of traffic citations by sex in Fayetteville. Male drivers received 60.3% of citations, female drivers 39.7%."
sex_and_citations |>
ggplot(aes(x = sex, y = pct_of_total_citations, fill = sex)) +
  geom_col() +
  labs(
    title = "Percentage of Citations by Sex in Fayetteville, NC",
    x = "Sex",
...
Based on the data, male drivers receive a substantially larger share of citations, accounting for 60.3% of all citations issued, while female drivers receive only 39.7%. This indicates that men are cited noticeably more frequently than women.
